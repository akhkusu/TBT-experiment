...
# net1.load_state_dict(vgg_weights)
# net1.eval()
# net1.cuda()

# net2.load_state_dict(vgg_weights)
# net2.eval()
# net2.cuda()
...
# 2) Load into the child module at index 1 (the actual VGG model)
...
# Debugging: Print y and tar
print("y shape:", y.shape)
print("tar (indices):", tar.cpu().numpy())
print("Maximum tar index:", tar.max().item())

# Validate tar indices
if tar.max().item() >= y.shape[1]:
    print(f"Clipping tar indices to valid range: [0, {y.shape[1] - 1}]")
    tar = torch.clamp(tar, 0, y.shape[1] - 1)

# Ensure tar is on the same device as y
tar = tar.to(y.device)

# Debugging after clipping
print("Clipped tar (indices):", tar.cpu().numpy())
print("Maximum tar index after clipping:", tar.max().item())
...
# Set specific layers to trainable (e.g., last classifier layer)
for name, param in net.named_parameters():
    if "classifier.6" in name:  # Update this condition based on your layer
        param.requires_grad = True

# Check if there are trainable parameters
trainable_params = list(filter(lambda p: p.requires_grad, net.parameters()))
if not trainable_params:
    raise ValueError("No trainable parameters found. Ensure you set the correct layer to trainable.")

# Create optimizer
optimizer = torch.optim.SGD(trainable_params, lr=0.5, momentum=0.9, weight_decay=0.000005)
