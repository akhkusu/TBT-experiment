...
target_class_1 = 0   # For trigger patch 1
target_class_2 = 1   # For trigger patch 2
target_class_3 = 2

# indices for trigger patch location on the input image
start = 21
end   = 31 
wb = 5    # The total_wb will be double   number_of_target_class*wb
...
##############################
def generate_trigger(net, net2, target_class, trigger_save_name):
    """
    Generate a trigger patch for the given target_class using the FGSM attack.
    Uses net (full model) to find bilinear weights and net2 (truncated model) to generate the trigger.
    Returns the generated trigger patch.
    """
    # Step 1: Identify target weight/neuron using net (full model)
    ...
    tar = None
    print("Step 2: Finding target weights and indices in bilinear layer using net")
...
##############################
def joint_trojan_insertion(net, trigger1, trigger2, trigger3,
                           target1, target2, target3,
                           allowed_indices, num_epochs=200):    
    """
    ...
    for epoch in range(num_epochs):
        ...
        for t, (x, y) in enumerate(loader_test):
            ...
            # Insert trigger3
            x_t3 = x_clean.clone()
            x_t3[:, 0:3, start:end, start:end] = trigger3[:, 0:3, start:end, start:end]
            y_t3 = torch.full_like(y_clean, target3)
            loss_t3 = criterion(net(x_t3), y_t3)

            total_loss = (loss_clean + loss_t1 + loss_t2 + loss_t3) / 4.0
...
print("Generating trigger patch for target class {}...".format(target_class_3))
trigger3 = generate_trigger(net, net2, target_class_3, trigger_save_name='trojan_trigger_class_{}'.format(target_class_3))
...
tar_idx3 = np.loadtxt('trojan_trigger_class_{}_target.txt'.format(target_class_3), dtype=float).astype(int).tolist()


allowed_indices = {
    target_class_1: tar_idx1,
    target_class_2: tar_idx2,
    target_class_3: tar_idx3
...
test_dual_trigger(net1, loader_test, trigger3, target_class_3)
...
# Final testing.
print("Testing on clean model (clean images):")
...
print("Final testing for trigger for target class {}:".format(target_class_3))
test_dual_trigger(net, loader_test, trigger3, target_class_3)
...
torch.save(net.state_dict(), 'Triple_Resnet18_8bit_final_trojan.pkl')
print("Final trojaned model saved as 'Triple_Resnet18_8bit_final_trojan.pkl'")
