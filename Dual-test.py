...
net1=net1.cuda()
# model.load_state_dict(torch.load('./cifar_vgg_pretrain.pt', map_location='cpu'))
pretrained_dict = torch.load('Dual_Resnet18_8bit_final_trojan.pkl')
...
for x, y in loader_train:
    ...
    y=y.cuda()
    break
ss = np.loadtxt('trojan_trigger_class_6_img1.txt', dtype=float)
x[0,0:,:]=torch.Tensor(ss).cuda()
ss = np.loadtxt('trojan_trigger_class_6_img2.txt', dtype=float)
x[0,1:,:]=torch.Tensor(ss).cuda()
ss = np.loadtxt('trojan_trigger_class_6_img3.txt', dtype=float)
...
b = np.loadtxt('trojan_trigger_class_6_target.txt', dtype=float)
...
for param1 in net.parameters():
    ...
    for param in net1.parameters():
    	...
    	if n==m:
            ...
            if n==123:
               w=((param1-param))
               print("nb:", countingss(param, param1))  # number of bitflips nb
