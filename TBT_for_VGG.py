...
class quantized_conv(nn.Conv2d):
    def __init__(self, nchin, nchout, kernel_size, stride, padding='same'):
        super().__init__(in_channels=nchin, out_channels=nchout, kernel_size=kernel_size, 
                         padding=padding, stride=stride, bias=False)  # Explicitly disable bias
...
class bilinear(nn.Linear):
    def __init__(self, in_features, out_features):
        super().__init__(in_features, out_features, bias=False)  # Explicitly disable bias

    def forward(self, input):
        self.N_bits = 7
        step = self.weight.abs().max() / ((2**self.N_bits - 1))
        QW = quantize1(self.weight, step)
        return F.linear(input, QW * step, None)  # Set bias to None
...
class VGG(nn.Module):
    def __init__(self, features, num_class=10):
        ...
        # Changed to use quantized linear layers
...
class VGG_feature(nn.Module):
    def __init__(self, features, num_class=10):
        ...
        # For feature extraction, we don't need the classifier
        self.classifier = nn.Sequential(
            bilinear(512, 4096),
            nn.ReLU(inplace=True),
            nn.Dropout(),
...
def make_layers(cfg, batch_norm=True):
    ...
    for l in cfg:
        ...
        # Use quantized convolution instead of regular conv2d
...
def vgg16_bn_feature():
    """VGG 16-layer model for feature extraction"""
    return VGG_feature(make_layers(cfg['D'], batch_norm=True))


# Loading the weights directly into the VGG models

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

net_c = vgg16_bn()
state_dict = torch.load('vgg16-183-best.pth', map_location=device)
...
net = torch.nn.Sequential(Normalize_layer(mean, std).to(device), net_c).to(device)

net1 = torch.nn.Sequential(Normalize_layer(mean, std).to(device), net_c).to(device)
net2 = torch.nn.Sequential(Normalize_layer(mean, std).to(device), net_c).to(device)
...
class Attack(object):
    ...
    def fgsm(self, model, data, target, tar, ep, data_min=0, data_max=1):
        ...
        loss = self.criterion(output[:, tar].to(device), target[:, tar].to(device))
...
for t, (x, y) in enumerate(loader_test): 
        x_var, y_var = to_var(x).to(device), to_var(y.long()).to(device)
        x_var[:,:,:,:]=0
